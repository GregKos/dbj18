    ...
    <form id="editform" action="{{route('posts.update', [$post->id])}}" method="post">
        ...
        <div class="form-row">
            <div class="form-group col-md-6">
                ...
                <input type="text" class="form-control" id="inputTitle" name="title" placeholder="The Post's Title" value="{{((old('title')) ? old('title') : $post->title)}}">
            </div>
            <div class="form-group col-md-6">
                <label for="inputSubtitle">Subtitle</label>
                <input type="text" class="form-control" id="inputSubtitle" name="subtitle" placeholder="A short, descriptive subtitle" value="{{((old('subtitle')) ? old('subtitle') : $post->subtitle)}}">
            </div>
        </div>
        <div class="form-group">
            <label for="inputContent">Content</label>
            <textarea class="form-control" id="inputContent" rows="3" name="content" placeholder="The full post body. This could be a WYSIWYG editor.">{{((old('content')) ? old('content') : $post->content)}}</textarea>
        </div>
        <div class="form-row">
            <div class="form-group col-md-6">
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <div class="form-group">
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="published" id="inlinePublished" value="1" @if((null === old('published') && $post->published) || old('published'))checked="checked"@endif>
                                <label class="form-check-label" for="inlinePublished">Yes</label>
                            </div>
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="published" id="inlineUnpublished" value="0" @if((null !== old('published') && intval(old('published')) === 0) || (null === old('published') && intval($post->published) === 0))checked="checked"@endif>
        ...
        <input type="hidden" name="filename" value="{{((old('filename')) ? old('filename') : $post->filename)}}" id="filefield">
...
<script>
    $(function () {
        $('#inputDate').datetimepicker({
            defaultDate: "{{((old('published_at')) ? old('published_at') : $post->published_at)}}",
    ...
    @if($post->filename || old('filename'))
    var mockFile = { name: "{{((old('filename')) ? old('filename') : $post->filename)}}", size: 0 };
    myAwesomeDropzone.emit("addedfile", mockFile);
    myAwesomeDropzone.emit("thumbnail", mockFile, "{{Storage::url('public/postfiles/'.((old('filename')) ? old('filename') : $post->filename))}}");
